...
(function ($) {
  ...
  $.fn.fitVids = function (options) {
    ...
    return this.each(function () {
      ...
      $allVideos = $allVideos.not("object object"); // Swfobj конфликтный патч
      $allVideos = $allVideos.not(ignoreList); // Отключите FitVids на этом видео.

      $allVideos.each(function () {
        var $this = $(this);
        if ($this.parents(ignoreList).length > 0) {
          return; // Отключите FitVids на этом видео.
        }
        if (
  ...
  // Внутренний счетчик для уникальных имён видео.
  $.fn.fitVids._count = 0;

  // Работы с jQuery или Zepto
